...
public class Rating extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	private final Logger logger = Logger.getLogger(Rating.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("			<script language=\"Javascript\" src=\"js/Rating.js\"></script> ");
				...
				if(session.getAttribute("ratingBean")!=null)
					ratingBean = (RatingBean)session.getAttribute("ratingBean");
				else
				...
				if(repAction.equals("V")){// external call for VIEW action
					Integer ratingID = Integer.parseInt(StringUtil.nullOrEmptyToString(rpo.getParameter("ratingID"), "0"));

					ratingList = adminBO.getRatingRecordListFiltered(ratingID, "", "", "", "");
					ratingBean= ratingList.get(0);
					
					session.setAttribute("ratingID", ratingID);
					...
					if (actionType.equals("new") ) {
						...
						session.removeAttribute("ratingBean");
						session.removeAttribute("ratingID");

						response.sendRedirect("RatingList?"+encryptedString);
						...
						ratingBean = submitActionReset(session);
				...
				constructPage(request, ratingBean, requestString, repAction, actionType);
	 ...
	 */
	private void constructPage(HttpServletRequest request, RatingBean ratingBean, String requestString, String repAction, String actionType) throws Exception{
...
//		String trxTypeStatus = StringUtil.nullToEmpty(ratingBean.getStatus());
...
//		pageConstructor.append("<input name=\"systemDate\" id=\"systemDate\" type=\"hidden\" onpaste=\"return false\" value=\""+ simpleDateFormat.format(new java.util.Date()) +"\" /> ");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Rating</legend>");
		...
		if(repAction.equals("V") || actionType.equals("edit")){
			...
			pageConstructor.append(StringUtil.nullToEmpty(ratingBean.getRatingCode()).toUpperCase());
			pageConstructor.append("                          				</span>");	
			pageConstructor.append("										<input type=\"hidden\" name=\"ratingCode\" id=\"ratingCode\" value=\""+StringUtil.nullToEmpty(ratingBean.getRatingCode())+"\">");
		} else{
			pageConstructor.append("										<input name=\"ratingCode\" id=\"ratingCode\" type=\"text\" required=\"required\" size=\"12\" maxlength=\"10\" size=\"4\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" onpaste=\"return false\" tabindex=\"1\" value=\""+StringUtil.nullToEmpty(ratingBean.getRatingCode())+"\" autofocus=\"autofocus\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("                   					<td width=\"10%\">");
		pageConstructor.append("										<span class=\"FrmCompTitle\">");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(ratingBean.getRatingDesc()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<input name=\"ratingDesc\" id=\"ratingDesc\" type=\"text\" required maxlength=\"30\" size=\"30\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" tabindex=\"2\" value=\""+StringUtil.nullToEmpty(ratingBean.getRatingDesc())+"\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								<tr>");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(ratingBean.getNote()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<textarea wrap=\"hard\" name=\"note\" id=\"note\" rows=\"2\" cols=\"50\" required=\"true\" onkeypress=\"return ( formIns.doLowSecurityFilter(event));\" tabindex=\"3\">"+StringUtil.nullToEmpty(ratingBean.getNote())+"</textarea>");
		}
		pageConstructor.append("									</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								</table>");
...
/*		if(!repAction.equals("N")){
			...
			parameterList.append("&ratingID="+transactionTypeBean.getTrxTypeID());
	...
	private ResultMessageBean submitAction(HttpServletRequest request, HttpSession session)
		...
		ratingVO.setRatingCode(StringUtil.nullToEmpty(request.getParameter("ratingCode")));
		ratingVO.setRatingDesc(StringUtil.nullToEmpty(request.getParameter("ratingDesc")));
		ratingVO.setNote(StringUtil.nullToEmpty(request.getParameter("note")));
		/**
		 * 
		 * 
		 * 
		...
		if(rootActionType.equals("edit")){//edit without creating history record
			...
			int ratingID = Integer.parseInt(StringUtil.nullOrEmptyToString(request.getSession().getAttribute("ratingID"), "0"));

			if(ratingID > 0){
				ratingVO.setRatingID(ratingID);
				isUpdated = adminBO.updateRating(ratingVO);

				if(isUpdated){
					logger.info("user: " + userID + " has successfully updated the ratingID: " + ratingID);
					ActivityLogVO activityLogVO = new ActivityLogVO();
					activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
					activityLogVO.setActivityDesc("user Updated rating successfully");
					activityLogVO.setActivityTypeID(ActivityType.EDITRATING_ACT.value());
					activityLogVO.setReference(new Integer(ratingID).toString());
					...
					resultMessageBean.setResultDescription("Success. ratingID: " + ratingID + " has been successfully updated");
				}else{
					logger.info("user: " + userID + " failed to update the ratingID: " + ratingID + ". no changes affected the system");
					resultMessageBean.setSucceeded(false);
					resultMessageBean.setResultDescription("Error At server side! Failed to updated ratingID: " + ratingID + "");
				}
			 ...
			 * CHECK IF THE RATING CODE TO BE ADDED IS UNIQUE IN THE SYSTEM	
			 */
		} else if(!rootActionType.equals("edit")){
			

			int ratingID = adminBO.addNewRating(ratingVO);	

			if(ratingID > 0){
				logger.info("user: " + userID + " has successfully added a new rating to the system. the new ratingID is: " + ratingID);

				ActivityLogVO activityLogVO = new ActivityLogVO();
				activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
				activityLogVO.setActivityDesc("user Updated rating successfully");
				activityLogVO.setActivityTypeID(ActivityType.ADDRATING_ACT.value());
				activityLogVO.setReference(new Integer(ratingID).toString());
				activityLogVO.setSessionID(request.getSession().getId());
				activityLogVO.setUserID(userID);					
				adminBO.logActivity(activityLogVO);
				...
				resultMessageBean.setSucceeded(true);
				resultMessageBean.setResultDescription("Success. ratingID: " + ratingID + " has been successfully stored in the system");					

			}else{
				logger.info("user: " + userID + " failed to add a new rating to the system. no changes affected the system");

				resultMessageBean.setSucceeded(false);
				resultMessageBean.setResultDescription("Error At server side! Failed to add rating. please try later");
			}
	...
	private ResultMessageBean deleteAction(HttpSession session)
		...
		Integer ratingID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("ratingID"), "0"));
		Integer userID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("userID"), "1"));


		boolean deleted = adminBO.removeRating(ratingID);

		if(deleted){
			logger.info("user: " + userID + " has successfully Deleted the rating: " + ratingID);

			ActivityLogVO activityLogVO = new ActivityLogVO();
			activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
			activityLogVO.setActivityDesc("user Deleted rating successfully");
			activityLogVO.setActivityTypeID(ActivityType.DELETERATING_ACT.value());
			activityLogVO.setReference(new Integer(ratingID).toString());
			...
			resultMessageBean.setSucceeded(true);
			resultMessageBean.setResultDescription("Success. ratingID: " + ratingID + " has been successfully Deleted");

		} else{
			logger.info("user: " + userID + ". Problem when Deleting rating: " + ratingID);

			resultMessageBean.setSucceeded(false);
			resultMessageBean.setResultDescription("Problem Deleting ratingID: " + ratingID + ". Please Try Later");
	 ...
	 * @throws Exception
	 */
	public RatingBean submitActionReset(HttpSession session)
			throws Exception{
	...
	private String constructBackButton(HttpServletRequest request, String parameterList, String userType, String reportAction, String actionType, String viewType){
		...
		if(!viewType.equals("H")){
			buttonConstructor.append("	<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" ");
			buttonConstructor.append("			onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='RatingList?"+encryptedParameterList+"';\" >");
