...
public class AddressType extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	private final Logger logger = Logger.getLogger(AddressType.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("			<script language=\"Javascript\" src=\"js/AddressType.js\"></script> ");
				...
				if(session.getAttribute("addressTypeBean")!=null)
					addressTypeBean = (AddressTypeBean)session.getAttribute("addressTypeBean");
				else
					session.setAttribute("addressTypeBean", addressTypeBean);
				...
				if(repAction.equals("V")){// external call for VIEW action
					Integer addressTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(rpo.getParameter("addressTypeID"), "0"));

					addressTypeList = adminBO.getAddressTypeRecordListFiltered(addressTypeID, "", "", "", "");
					addressTypeBean= addressTypeList.get(0);
					
					session.setAttribute("addressTypeID", addressTypeID);
					...
					if (actionType.equals("new") ) {
						...
						session.removeAttribute("addressTypeBean");
						session.removeAttribute("addressTypeID");

						response.sendRedirect("AddressTypeList?"+encryptedString);
						...
						addressTypeBean = submitActionReset(session);
				...
				setTitle("Address Type");

				constructPage(request, addressTypeBean, requestString, repAction, actionType);
	 ...
	 */
	private void constructPage(HttpServletRequest request, AddressTypeBean addressTypeBean, String requestString, String repAction, String actionType) throws Exception{
...
//		String trxTypeStatus = StringUtil.nullToEmpty(addressTypeBean.getStatus());
...
//		pageConstructor.append("<input name=\"systemDate\" id=\"systemDate\" type=\"hidden\" onpaste=\"return false\" value=\""+ simpleDateFormat.format(new java.util.Date()) +"\" /> ");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Address Type</legend>");
		...
		if(repAction.equals("V") || actionType.equals("edit")){
			...
			pageConstructor.append(StringUtil.nullToEmpty(addressTypeBean.getAddrTypeCode()).toUpperCase());
			pageConstructor.append("                          				</span>");	
			pageConstructor.append("										<input type=\"hidden\" name=\"addressTypeCode\" id=\"addressTypeCode\" value=\""+StringUtil.nullToEmpty(addressTypeBean.getAddrTypeCode())+"\">");
		} else{
			pageConstructor.append("										<input name=\"addressTypeCode\" id=\"addressTypeCode\" type=\"text\" required=\"required\" size=\"12\" maxlength=\"10\" size=\"4\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" onpaste=\"return false\" tabindex=\"1\" value=\""+StringUtil.nullToEmpty(addressTypeBean.getAddrTypeCode())+"\" autofocus=\"autofocus\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("                   					<td width=\"10%\">");
		pageConstructor.append("										<span class=\"FrmCompTitle\">");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(addressTypeBean.getAddrTypeDesc()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<input name=\"addressTypeDesc\" id=\"addressTypeDesc\" type=\"text\" required maxlength=\"30\" size=\"30\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" tabindex=\"2\" value=\""+StringUtil.nullToEmpty(addressTypeBean.getAddrTypeDesc())+"\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								<tr>");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(addressTypeBean.getNote()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<textarea wrap=\"hard\" name=\"note\" id=\"note\" rows=\"2\" cols=\"50\" required=\"true\" onkeypress=\"return ( formIns.doLowSecurityFilter(event));\" tabindex=\"3\">"+StringUtil.nullToEmpty(addressTypeBean.getNote())+"</textarea>");
		}
		pageConstructor.append("									</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								</table>");
...
/*		if(!repAction.equals("N")){
			...
			parameterList.append("&addressTypeID="+transactionTypeBean.getTrxTypeID());
	...
	private ResultMessageBean submitAction(HttpServletRequest request, HttpSession session)
		...
		addressTypeVO.setAddrTypeCode(StringUtil.nullToEmpty(request.getParameter("addressTypeCode")));
		addressTypeVO.setAddrTypeDesc(StringUtil.nullToEmpty(request.getParameter("addressTypeDesc")));
		addressTypeVO.setNote(StringUtil.nullToEmpty(request.getParameter("note")));
		/**
		 * 
		 * 
		 * 
		...
		if(rootActionType.equals("edit")){//edit without creating history record
			...
			int addressTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(request.getSession().getAttribute("addressTypeID"), "0"));

			if(addressTypeID > 0){
				addressTypeVO.setAddrTypeID(addressTypeID);
				isUpdated = adminBO.updateAddressType(addressTypeVO);

				if(isUpdated){
					logger.info("user: " + userID + " has successfully updated the addressTypeID: " + addressTypeID);
					ActivityLogVO activityLogVO = new ActivityLogVO();
					activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
					activityLogVO.setActivityDesc("user Updated addressType successfully");
					activityLogVO.setActivityTypeID(ActivityType.EDITADDRESSTYPE_ACT.value());
					activityLogVO.setReference(new Integer(addressTypeID).toString());
					...
					resultMessageBean.setResultDescription("Success. addressTypeID: " + addressTypeID + " has been successfully updated");
				}else{
					logger.info("user: " + userID + " failed to update the addressTypeID: " + addressTypeID + ". no changes affected the system");
					resultMessageBean.setSucceeded(false);
					resultMessageBean.setResultDescription("Error At server side! Failed to updated addressTypeID: " + addressTypeID + "");
				}
			...
			int addressTypeID = adminBO.addNewAddressType(addressTypeVO);	

			if(addressTypeID > 0){
				logger.info("user: " + userID + " has successfully added a new addressType to the system. the new addressTypeID is: " + addressTypeID);

				ActivityLogVO activityLogVO = new ActivityLogVO();
				activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
				activityLogVO.setActivityDesc("user Updated addressType successfully");
				activityLogVO.setActivityTypeID(ActivityType.ADDADDRESSTYPE_ACT.value());
				activityLogVO.setReference(new Integer(addressTypeID).toString());
				activityLogVO.setSessionID(request.getSession().getId());
				activityLogVO.setUserID(userID);					
				adminBO.logActivity(activityLogVO);
				...
				resultMessageBean.setSucceeded(true);
				resultMessageBean.setResultDescription("Success. addressTypeID: " + addressTypeID + " has been successfully stored in the system");					

			}else{
				logger.info("user: " + userID + " failed to add a new addressType to the system. no changes affected the system");

				resultMessageBean.setSucceeded(false);
				resultMessageBean.setResultDescription("Error At server side! Failed to add addressType. please try later");
			}
	...
	private ResultMessageBean deleteAction(HttpSession session)
		...
		Integer addressTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("addressTypeID"), "0"));
		Integer userID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("userID"), "1"));


		boolean deleted = adminBO.removeAddressType(addressTypeID);

		if(deleted){
			logger.info("user: " + userID + " has successfully Deleted the addressType: " + addressTypeID);

			ActivityLogVO activityLogVO = new ActivityLogVO();
			activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
			activityLogVO.setActivityDesc("user Deleted addressType successfully");
			activityLogVO.setActivityTypeID(ActivityType.DELETEADDRESSTYPE_ACT.value());
			activityLogVO.setReference(new Integer(addressTypeID).toString());
			...
			resultMessageBean.setSucceeded(true);
			resultMessageBean.setResultDescription("Success. addressTypeID: " + addressTypeID + " has been successfully Deleted");

		} else{
			logger.info("user: " + userID + ". Problem when Deleting addressType: " + addressTypeID);

			resultMessageBean.setSucceeded(false);
			resultMessageBean.setResultDescription("Problem Deleting addressTypeID: " + addressTypeID + ". Please Try Later");
	 ...
	 * @throws Exception
	 */
	public AddressTypeBean submitActionReset(HttpSession session)
			throws Exception{
	...
	private String constructBackButton(HttpServletRequest request, String parameterList, String userType, String reportAction, String actionType, String viewType){
		...
		if(!viewType.equals("H")){
			buttonConstructor.append("	<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" ");
			buttonConstructor.append("			onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='AddressTypeList?"+encryptedParameterList+"';\" >");
