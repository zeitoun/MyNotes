...
public class PhoneType extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	private final Logger logger = Logger.getLogger(PhoneType.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("			<script language=\"Javascript\" src=\"js/PhoneType.js\"></script> ");
				...
				if(session.getAttribute("phoneTypeBean")!=null)
					phoneTypeBean = (PhoneTypeBean)session.getAttribute("phoneTypeBean");
				else
					session.setAttribute("phoneTypeBean", phoneTypeBean);
				...
				if(repAction.equals("V")){// external call for VIEW action
					Integer phoneTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(rpo.getParameter("phoneTypeID"), "0"));

					phoneTypeList = adminBO.getPhoneTypeRecordListFiltered(phoneTypeID, "", "", "", "");
					phoneTypeBean= phoneTypeList.get(0);
					
					session.setAttribute("phoneTypeID", phoneTypeID);
					...
					if (actionType.equals("new") ) {
						...
						session.removeAttribute("phoneTypeBean");
						session.removeAttribute("phoneTypeID");

						response.sendRedirect("PhoneTypeList?"+encryptedString);
						...
						phoneTypeBean = submitActionReset(session);
				...
				constructPage(request, phoneTypeBean, requestString, repAction, actionType);
	 ...
	 */
	private void constructPage(HttpServletRequest request, PhoneTypeBean phoneTypeBean, String requestString, String repAction, String actionType) throws Exception{
...
//		String trxTypeStatus = StringUtil.nullToEmpty(phoneTypeBean.getStatus());
...
//		pageConstructor.append("<input name=\"systemDate\" id=\"systemDate\" type=\"hidden\" onpaste=\"return false\" value=\""+ simpleDateFormat.format(new java.util.Date()) +"\" /> ");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Phone Type</legend>");
		...
		if(repAction.equals("V") || actionType.equals("edit")){
			...
			pageConstructor.append(StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeCode()).toUpperCase());
			pageConstructor.append("                          				</span>");	
			pageConstructor.append("										<input type=\"hidden\" name=\"phoneTypeCode\" id=\"phoneTypeCode\" value=\""+StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeCode())+"\">");
		} else{
			pageConstructor.append("										<input name=\"phoneTypeCode\" id=\"phoneTypeCode\" type=\"text\" required=\"required\" size=\"12\" maxlength=\"10\" size=\"4\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" onpaste=\"return false\" tabindex=\"1\" value=\""+StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeCode())+"\" autofocus=\"autofocus\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("                   					<td width=\"10%\">");
		pageConstructor.append("										<span class=\"FrmCompTitle\">");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeDesc()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<input name=\"phoneTypeDesc\" id=\"phoneTypeDesc\" type=\"text\" required maxlength=\"30\" size=\"30\" autocomplete=\"off\" onkeypress=\"return formIns.doLowSecurityFilter(event)\" tabindex=\"2\" value=\""+StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeDesc())+"\" /> ");
		}
		pageConstructor.append("                           			</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								<tr>");
		...
		if(repAction.equals("V")){
			pageConstructor.append("										<span class=\"note\">");
			pageConstructor.append(StringUtil.nullToEmpty(phoneTypeBean.getNote()).toUpperCase());
			pageConstructor.append("                           				</span>");
		} else{
			pageConstructor.append("										<textarea wrap=\"hard\" name=\"note\" id=\"note\" rows=\"2\" cols=\"50\" required=\"true\" onkeypress=\"return ( formIns.doLowSecurityFilter(event));\" tabindex=\"3\">"+StringUtil.nullToEmpty(phoneTypeBean.getNote())+"</textarea>");
		}
		pageConstructor.append("									</td>");
		pageConstructor.append("								</tr>");
		pageConstructor.append("								</table>");
...
/*		if(!repAction.equals("N")){
			...
			parameterList.append("&phoneTypeID="+transactionTypeBean.getTrxTypeID());
	...
	private ResultMessageBean submitAction(HttpServletRequest request, HttpSession session)
		...
		PhoneTypeVO phoneTypeVO = new PhoneTypeVO();
		Integer userID = Integer.parseInt(StringUtil.nullToString(session.getAttribute("userID"), "0"));		


		phoneTypeVO.setPhoneTypeCode(StringUtil.nullToEmpty(request.getParameter("phoneTypeCode")));
		phoneTypeVO.setPhoneTypeDesc(StringUtil.nullToEmpty(request.getParameter("phoneTypeDesc")));
		phoneTypeVO.setNote(StringUtil.nullToEmpty(request.getParameter("note")));
		/**
		 * 
		 * 
		 * 
		...
		if(rootActionType.equals("edit")){//edit without creating history record
			...
			int phoneTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(request.getSession().getAttribute("phoneTypeID"), "0"));

			if(phoneTypeID > 0){
				phoneTypeVO.setPhoneTypeID(phoneTypeID);
				isUpdated = adminBO.updatePhoneType(phoneTypeVO);

				if(isUpdated){
					logger.info("user: " + userID + " has successfully updated the phoneTypeID: " + phoneTypeID);
					ActivityLogVO activityLogVO = new ActivityLogVO();
					activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
					activityLogVO.setActivityDesc("user Updated phoneType successfully");
					activityLogVO.setActivityTypeID(ActivityType.EDITPHONETYPE_ACT.value());
					activityLogVO.setReference(new Integer(phoneTypeID).toString());
					...
					resultMessageBean.setResultDescription("Success. phoneTypeID: " + phoneTypeID + " has been successfully updated");
				}else{
					logger.info("user: " + userID + " failed to update the phoneTypeID: " + phoneTypeID + ". no changes affected the system");
					resultMessageBean.setSucceeded(false);
					resultMessageBean.setResultDescription("Error At server side! Failed to updated phoneTypeID: " + phoneTypeID + "");
				}
			 ...
			 * CHECK IF THE PHONETYPE CODE TO BE ADDED IS UNIQUE IN THE SYSTEM	
			 */
		} else if(!rootActionType.equals("edit")){
			

			int phoneTypeID = adminBO.addNewPhoneType(phoneTypeVO);	

			if(phoneTypeID > 0){
				logger.info("user: " + userID + " has successfully added a new phoneType to the system. the new phoneTypeID is: " + phoneTypeID);

				ActivityLogVO activityLogVO = new ActivityLogVO();
				activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
				activityLogVO.setActivityDesc("user Updated phoneType successfully");
				activityLogVO.setActivityTypeID(ActivityType.ADDPHONETYPE_ACT.value());
				activityLogVO.setReference(new Integer(phoneTypeID).toString());
				activityLogVO.setSessionID(request.getSession().getId());
				activityLogVO.setUserID(userID);					
				adminBO.logActivity(activityLogVO);
				...
				resultMessageBean.setSucceeded(true);
				resultMessageBean.setResultDescription("Success. phoneTypeID: " + phoneTypeID + " has been successfully stored in the system");					

			}else{
				logger.info("user: " + userID + " failed to add a new phoneType to the system. no changes affected the system");

				resultMessageBean.setSucceeded(false);
				resultMessageBean.setResultDescription("Error At server side! Failed to add phoneType. please try later");
			}
	...
	private ResultMessageBean deleteAction(HttpSession session)
		...
		Integer phoneTypeID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("phoneTypeID"), "0"));
		Integer userID = Integer.parseInt(StringUtil.nullOrEmptyToString(session.getAttribute("userID"), "1"));


		boolean deleted = adminBO.removePhoneType(phoneTypeID);

		if(deleted){
			logger.info("user: " + userID + " has successfully Deleted the phoneType: " + phoneTypeID);

			ActivityLogVO activityLogVO = new ActivityLogVO();
			activityLogVO.setActivityDate(new Timestamp(new java.util.Date().getTime()));
			activityLogVO.setActivityDesc("user Deleted phoneType successfully");
			activityLogVO.setActivityTypeID(ActivityType.DELETEPHONETYPE_ACT.value());
			activityLogVO.setReference(new Integer(phoneTypeID).toString());
			...
			resultMessageBean.setSucceeded(true);
			resultMessageBean.setResultDescription("Success. phoneTypeID: " + phoneTypeID + " has been successfully Deleted");

		} else{
			logger.info("user: " + userID + ". Problem when Deleting phoneType: " + phoneTypeID);

			resultMessageBean.setSucceeded(false);
			resultMessageBean.setResultDescription("Problem Deleting phoneTypeID: " + phoneTypeID + ". Please Try Later");
	 ...
	 * @throws Exception
	 */
	public PhoneTypeBean submitActionReset(HttpSession session)
			throws Exception{

		session.removeAttribute("phoneTypeBean");
		session.removeAttribute("phoneTypeID");
		return new PhoneTypeBean();
	}
	/**
	 * 
	 * 
	...
	private String constructBackButton(HttpServletRequest request, String parameterList, String userType, String reportAction, String actionType, String viewType){
		...
		if(!viewType.equals("H")){
			buttonConstructor.append("	<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" ");
			buttonConstructor.append("			onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='PhoneTypeList?"+encryptedParameterList+"';\" >");
