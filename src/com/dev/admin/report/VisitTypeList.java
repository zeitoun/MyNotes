...
public class VisitTypeList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(VisitTypeList.class);
	private final Logger logger = Logger.getLogger(VisitTypeList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/VisitTypeList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "VISIT_TYPE_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "VISIT_TYPE_DESC";
					}

					visitTypeList = adminBO.getVisitTypeRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					visitTypeList = adminBO.getVisitTypeRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("VisitTypeList");
				setValueBean("visitTypeListBean");

				constructPage(request, session, response, visitTypeList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<VisitTypeBean> visitTypeList, 
		...
		logger.info("\nvisitType: " + visitTypeList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, visitTypeList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By VisitType Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by VisitType Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Visit Type List</legend>");
		...
		if (visitTypeList!=null && visitTypeList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < visitTypeList.size()); i++) {
				visitTypeBean = (VisitTypeBean) visitTypeList.get(i);
				...
				viewDetailsLinkParameters.append("&visitTypeID="+visitTypeBean.getVisitTypeID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"VisitType?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Visit Type details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(visitTypeBean.getVisitTypeCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(visitTypeBean.getVisitTypeDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (visitTypeList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, visitTypeList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='VisitType?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
