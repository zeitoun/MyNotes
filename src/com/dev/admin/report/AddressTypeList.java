...
public class AddressTypeList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(AddressTypeList.class);
	private final Logger logger = Logger.getLogger(AddressTypeList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/AddressTypeList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "ADDRESS_TYPE_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "ADDRESS_TYPE_DESC";
					}

					addressTypeList = adminBO.getAddressTypeRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					addressTypeList = adminBO.getAddressTypeRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("AddressTypeList");
				setValueBean("addressTypeListBean");

				constructPage(request, session, response, addressTypeList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<AddressTypeBean> addressTypeList, 
		...
		logger.info("\naddressType: " + addressTypeList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, addressTypeList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By AddressType Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by AddressType Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Address Type List</legend>");
		...
		if (addressTypeList!=null && addressTypeList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < addressTypeList.size()); i++) {
				...
				viewDetailsLinkParameters.append("&addressTypeID="+addressTypeBean.getAddrTypeID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"AddressType?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Address Type details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(addressTypeBean.getAddrTypeCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(addressTypeBean.getAddrTypeDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (addressTypeList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, addressTypeList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='AddressType?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
