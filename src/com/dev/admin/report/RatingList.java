...
public class RatingList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(RatingList.class);
	private final Logger logger = Logger.getLogger(RatingList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/RatingList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "RATING_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "RATING_DESC";
					}

					ratingList = adminBO.getRatingRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					ratingList = adminBO.getRatingRecordListFiltered(null, null, null, sort, direction);
				...
				setTitle("RatingList");
				setValueBean("ratingListBean");

				constructPage(request, session, response, ratingList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<RatingBean> ratingList, 
		...
		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, ratingList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By Rating Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by Rating Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Rating List</legend>");
		...
		if (ratingList!=null && ratingList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < ratingList.size()); i++) {
				...
				viewDetailsLinkParameters.append("&ratingID="+ratingBean.getRatingID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"Rating?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Rating details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(ratingBean.getRatingCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(ratingBean.getRatingDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (ratingList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, ratingList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='Rating?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
