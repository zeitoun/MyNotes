...
public class PersonTitleList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PersonTitleList.class);
	private final Logger logger = Logger.getLogger(PersonTitleList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/PersonTitleList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "TITLE_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "TITLE_DESC";
					}

					personTitleList = adminBO.getPersonTitleRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					personTitleList = adminBO.getPersonTitleRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("PersonTitleList");
				setValueBean("personTitleListBean");

				constructPage(request, session, response, personTitleList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<PersonTitleBean> personTitleList, 
		...
		logger.info("\npersonTitle: " + personTitleList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, personTitleList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By PersonTitle Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by PersonTitle Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		if (personTitleList!=null && personTitleList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < personTitleList.size()); i++) {
				personTitleBean = (PersonTitleBean) personTitleList.get(i);
				...
				viewDetailsLinkParameters.append("&personTitleID="+personTitleBean.getPersonTitleID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"PersonTitle?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Person Title details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(personTitleBean.getPersonTitleCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(personTitleBean.getPersonTitleDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (personTitleList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, personTitleList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='PersonTitle?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
