...
public class ProfessionList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(ProfessionList.class);
	private final Logger logger = Logger.getLogger(ProfessionList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/ProfessionList.js\"></script>");
				...
				if(formAction.equals("Sort")){
					if (sort != null && sort.equals("4")) {
						...
						direction = NavigationSortingUtil.getSortingDirection("proFieldCode", request);
					}  
				}
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "PROFESSION_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "PROFESSION_DESC";
						else if(criteriaTypeID.equals("3")) // title
							criteriaType = "PRO_FIELD_CODE";
						
					}

					professionList = adminBO.getProfessionRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					professionList = adminBO.getProfessionRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("ProfessionList");
				setValueBean("professionListBean");

				constructPage(request, session, response, professionList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<ProfessionBean> professionList, 
		...
		logger.info("\nprofession: " + professionList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, professionList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By Profession Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by Profession Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
		pageConstructor.append("							<option title=\"Search by Professional Field\" value=\"3\" "+(criteriaTypeID.equals("3") ? " selected" : "")+" >Professional Field</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Profession List</legend>");
		...
		if (professionList!=null && professionList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < professionList.size()); i++) {
				professionBean = (ProfessionBean) professionList.get(i);
				...
				viewDetailsLinkParameters.append("&professionID="+professionBean.getProfessionID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"Profession?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Profession details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(professionBean.getProfessionCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(professionBean.getProfessionDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(professionBean.getProFieldBean().getProFieldCode()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (professionList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, professionList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='Profession?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
