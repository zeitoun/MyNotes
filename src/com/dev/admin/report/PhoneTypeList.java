...
public class PhoneTypeList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(PhoneTypeList.class);
	private final Logger logger = Logger.getLogger(PhoneTypeList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/PhoneTypeList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "PHONE_TYPE_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "PHONE_TYPE_DESC";
					}

					phoneTypeList = adminBO.getPhoneTypeRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					phoneTypeList = adminBO.getPhoneTypeRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("PhoneTypeList");
				setValueBean("phoneTypeListBean");

				constructPage(request, session, response, phoneTypeList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<PhoneTypeBean> phoneTypeList, 
		...
		logger.info("\nphoneType: " + phoneTypeList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, phoneTypeList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By PhoneType Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by PhoneType Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Phone Type List</legend>");
		...
		if (phoneTypeList!=null && phoneTypeList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < phoneTypeList.size()); i++) {
				phoneTypeBean = (PhoneTypeBean) phoneTypeList.get(i);
				...
				viewDetailsLinkParameters.append("&phoneTypeID="+phoneTypeBean.getPhoneTypeID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"PhoneType?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Phone Type details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(phoneTypeBean.getPhoneTypeDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (phoneTypeList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, phoneTypeList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='PhoneType?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
