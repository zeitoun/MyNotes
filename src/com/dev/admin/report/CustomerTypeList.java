...
public class CustomerTypeList extends PageSecureServlet {
	/**
	 * 
	 * 
	 * 
	...
	//	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(CustomerTypeList.class);
	private final Logger logger = Logger.getLogger(CustomerTypeList.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			if(session.getAttribute("userID")!= null ){
				...
				scriptIncludeBuffer.append("	<script language=\"Javascript\" src=\"js/CustomerTypeList.js\"></script>");
				...
				if (actionType.equals("search") ) {
					...
					if(!criteriaTypeID.equals("")){
						...
						if(criteriaTypeID.equals("1")) // code
							criteriaType = "CUST_TYPE_CODE";
						else if(criteriaTypeID.equals("2")) // title
							criteriaType = "CUST_TYPE_DESC";
					}

					customerTypeList = adminBO.getCustTypeRecordListFiltered(null, criteriaType, criteriaValue, sort, direction);


				}else {

					customerTypeList = adminBO.getCustTypeRecordListFiltered(null, null, null, sort, direction);

				}



				setTitle("CustomerTypeList");
				setValueBean("customerTypeListBean");

				constructPage(request, session, response, customerTypeList, rpo, sort, direction, actionType, criteriaTypeID, criteriaValue);
			}
	 ...
	 * @param criteriaValue
	 */
	public void constructPage(HttpServletRequest request, HttpSession session, HttpServletResponse response, ArrayList<CustomerTypeBean> customerTypeList, 
		...
		logger.info("\ncustomerType: " + customerTypeList.size());

		NavigationBean limits = NavigationSortingUtil.getNavigationLimits(currentPageNum, numOfRecordToShow, customerTypeList.size());
...
//		pageConstructor.append("							<option title=\"\" value=\"0\"></option>");
		pageConstructor.append("							<option title=\"Search By CustomerType Code\" value=\"1\" "+(criteriaTypeID.equals("1") ? " selected" : "")+" >Code</option>");
		pageConstructor.append("							<option title=\"Search by CustomerType Description\" value=\"2\" "+(criteriaTypeID.equals("2") ? " selected" : "")+" >Title</option>");
...
//		pageConstructor.append("<tr>");
		...
		pageConstructor.append("			<legend class=\"mainFSet\" >Customer Type List</legend>");
		...
		if (customerTypeList!=null && customerTypeList.size() > 0) {
			...
			for(int i = startNumber; (i < endNumber) && (i < customerTypeList.size()); i++) {
				customerTypeBean = (CustomerTypeBean) customerTypeList.get(i);
				...
				viewDetailsLinkParameters.append("&customerTypeID="+customerTypeBean.getCustTypeID());
				...
				pageConstructor.append("						<a class=\"linkStyle\" href=\"CustomerType?"+encryptedViewDetailsLinkParameters+"\" title=\"Click Here to view Customer Type details\" >");
				pageConstructor.append(StringUtil.nullToEmpty(customerTypeBean.getCustTypeCode()));
				pageConstructor.append("						</a>");
				pageConstructor.append("					</td>");
				pageConstructor.append("					<td class=\"stringStyleEng\">");
				pageConstructor.append(StringUtil.nullToEmpty(customerTypeBean.getCustTypeDesc()));
				pageConstructor.append("					</td>");
				pageConstructor.append("				</tr>");
			}
		}else {
		...
		pageConstructor.append("			</td>");
		pageConstructor.append("		</tr>");
		if (customerTypeList.size() > numOfRecordToShow ) {
			pageConstructor.append("		<tr>");
			pageConstructor.append("			<td >");
			...
			pageConstructor.append(HTMLDesign.getNavigationBar(request, staticNavigationParameterList.toString(), currentPageNum, customerTypeList.size(), endNumber, maxNumPage, this.getServletName(), "pageNumber",clientDirName, "Eng"));
		...
		if(userRole.equals("1")){
			...
			pageConstructor.append("					<br />");
			pageConstructor.append("					<div align=\"center\">");
			pageConstructor.append("					<a href=\"#\" class=\"btn_large btn_blue btn_awesome\" rel=\"nofollow\" onClick=\"this.disabled='disabled';createLoadingImg();window.location.href='CustomerType?"+encryptedViewDetailsLinkParameters+"'\">ADD NEW</a>");
			pageConstructor.append("					</div>");
			pageConstructor.append("				</td>");
			pageConstructor.append("			</tr>");
			pageConstructor.append("			</table>");
